...
## Angular Hybrid Encoding

Angular Hybrid encoding advances quantum data embedding by interleaving angular rotations with complementary encoding techniques to harness their collective strengths while mitigating individual weaknesses. At its core, Angular Hybrid first maps continuous classical features into rotation angles for single-qubit RY (or RX) gates, ensuring hardware-efficient state preparation with minimal circuit depth—an essential requirement for Noisy Intermediate-Scale Quantum (NISQ) devices. To enhance expressivity beyond what pure angular encoding offers, this method then layers in amplitude or phase encoding: amplitude encoding compresses large feature vectors into the quantum state’s probability amplitudes, achieving exponential data packing, while phase encoding injects additional degrees of freedom by rotating about the Z axis. Concretely, each feature vector is partitioned into blocks, where an initial block’s values determine RY rotations on designated qubits, followed by controlled-RZ gates that phase-encode a second block, and finally an amplitude-encoding subcircuit that normalizes and loads a third block of data into the state amplitudes. This sequential mechanism yields a quantum feature map that remains shallow yet rich in representational capacity. The hybrid construction preserves local data correlations through angular rotations, captures global relationships via amplitude compression, and fine-tunes relative feature importance with phase shifts. Consequently, Angular Hybrid encoding delivers superior state distinguishability, avoids concentration into nearly mixed states, and maintains efficient qubit usage. Empirically, hybrid-encoded quantum classifiers demonstrate improved feature separability in Hilbert space and higher accuracy on tasks like QCNN-based image classification, outperforming single-encoding schemes under comparable resource constraints.
...
Hur et al. (2022), addresses the challenge of harnessing quantum machine learning for classical image classification by proposing a fully parameterized Quantum Convolutional Neural Network (QCNN) architecture that relies exclusively on two-qubit interactions and shallow-depth circuits suitable for NISQ devices. They adopt a variational quantum circuit approach inspired by classical CNNs, embedding classical data through several encoding methods—amplitude, qubit, dense, and newly introduced hybrid direct and hybrid angle encodings—and pre-processing via bilinear interpolation, PCA, or autoencoders to match the limited qubit resources. The authors benchmark their QCNN designs on the binary MNIST and Fashion MNIST tasks—each converted into 8- or 16-dimensional inputs—using a variety of convolutional “ansatz” circuits, pooling operations, cost functions (mean squared error and cross-entropy), and optimizers (Adam and Nesterov). Across models ranging from 12 to 51 trainable parameters, the QCNN consistently achieves superior classification accuracy (up to 99% on MNIST and 94% on Fashion MNIST) compared to classical CNNs with comparable parameter counts, while also demonstrating faster convergence and reduced sensitivity to parameter initialization. The study finds that translationally invariant, two-qubit layers suffice to capture both local and global correlations via entanglement, that hybrid angle encoding overcomes normalization issues inherent in direct encodings, and that open-boundary QCNNs preserve performance while easing hardware connectivity demands. These results substantiate the QCNN’s promise for efficient, scalable classical data classification on near-term quantum hardware.
...
Hur, T., Kim, L., & Park, D. K. (2022). Quantum convolutional neural network for classical data classification. arXiv preprint arXiv:2108.00661.